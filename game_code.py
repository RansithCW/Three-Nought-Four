    ...
    def bid(self, current_bid):
        if self.is_human:
            try:
                ...
                return 0
            return bid if bid > current_bid else 0
        else:
            bot_bid = random.choice([0, current_bid + 10]) if current_bid < 160 else 0
    ...
    def play_card(self, lead_suit, trump_suit):
        if self.is_human:
            ...
            if legal_cards:
                ...
                card = max(trump_cards) if trump_cards else self.hand[0] 
                # lowest card if can't answer trick
            self.hand.remove(card)
            return card
...
class Game:
    def __init__(self):
        self.players = [Player(f"Player {i+1}", is_human=(i == 0)) for i in range(4)]
    ...
    def play_trick(self, lead_index: int=0):
        ...
        for i in range(4):
            ...
            if i == 0:
                lead_suit = card.suit
            trick.append((card, (lead_index + i) % 4))
            print(f"{'\n' if i==0 else ''}{player.name} plays {card}")

        print("TABLE:", [str(card) for card, player_id in trick], "\n")
        ...
        winner_id = max(trick, key=lambda x: card_value(x[0]))[1] # index of winner
        winner = self.players[winner_id]
        print(f"Trick won by {winner.name}\n")
        winner.tricks.extend([card for card, _ in trick])
        return winner_id
